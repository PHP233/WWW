 ...
 * This scripts is distinct from libraries/common.inc.php because this
 * scripts is called from /test.
 *
 * @package PhpMyAdmin
 */
 ...
 * ends scripts execution and closes session
 *
 ...
 * it ignores optionaly query paramters in $page (scripts.php?ignored)
 *
...
{
    if (strpos($url, '?') === false) {
        return '<scripts src="./js/' . $url . '?ts=' . filemtime('./js/' . $url) . '" type="text/javascript"></scripts>' . "\n";
    } else {
        return '<scripts src="./js/' . $url . '" type="text/javascript"></scripts>' . "\n";
    }
}
