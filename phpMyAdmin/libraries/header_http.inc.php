...
if (!$GLOBALS['cfg']['AllowThirdPartyFraming']) {
    ...
    header("X-Content-Security-Policy: allow 'self' ; options inline-scripts eval-scripts; frame-ancestors 'self'; img-src 'self' data:");
    if (PMA_USR_BROWSER_AGENT == 'SAFARI' && PMA_USR_BROWSER_VER < '6.0.0') {
        header("X-WebKit-CSP: allow 'self'; options inline-scripts eval-scripts");
    } else {
