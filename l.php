...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  if ($_POST['act'] == 'MySQL检测') {
  	if(function_exists("mysql_close")==1) {
  		...
  		if ($link){
  			echo "<scripts>alert('连接到MySql数据库正常')</scripts>";
  		} else {
  			echo "<scripts>alert('无法连接到MySql数据库！')</scripts>";
  		}
  	} else {
  		echo "<scripts>alert('服务器不支持MySQL数据库！')</scripts>";
  	}
  }
	?>
<!--函数检测-->
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <?php
  if ($_POST['act'] == '函数检测') {
  	echo "<scripts>alert('$funRe')</scripts>";
  }
  ?>
</table>
